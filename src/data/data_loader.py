...
from .data_sets import FilteredDataSet, ShuffledDataSet
from .tranforms import MinMaxScaler
from .data_set_samplers import DataSetSamplers
from ..config import DATA_DIR, FILTERED_DATA_SET_NAME, NUM_COLS, BATCH_SIZE, NUM_WORKERS

df = pd.read_csv(os.path.join(DATA_DIR, FILTERED_DATA_SET_NAME))
print('Dataframe shape: ', df.shape)
np_array = df.to_numpy()
transform = MinMaxScaler.from_features(np_array[:, :np_array.shape[1] - 1])

data_set = FilteredDataSet(np_array, transform)
